...
/**
 * Represents an Edge computing node in the simulation.
 * Edge nodes are located close to IoT devices and provide low-latency computing resources.
 */
public class EdgeNode extends Device {
     ...
     * @param maxConnections Maximum number of devices that can connect to this edge node
     ...
     */
    public EdgeNode(String id, String name, double xPos, double yPos,
        ...
        super(id, DeviceType.EDGE_NODE, name, xPos, yPos, processingPower, memory, storage, batteryCapacity);
    ...
    public boolean connectDevice(Device device) {
        // Check if the edge node is active
        if (!isActive) {
            return false;
        }
        
        // Check if the edge node has reached its maximum connections
    ...
    /**
     * Queues a task for execution on this edge node
     * 
    ...
    public boolean queueTask(Task task) {
        ...
        // Check if the edge node has enough resources to eventually execute the task
    ...
    public boolean executeTask(Task task) {
        ...
        // Check if the edge node has enough resources
        if (task.getCpuRequirement() > processingPower ||
            task.getRamRequirement() > memory ||
            task.getStorageRequirement() > storage) {
            return false;
    ...
    /**
     * Calculates the cost of executing a task on this edge node
     * 
    ...
    /**
     * Returns a string representation of the edge node
     * 
     * @return String representation of the edge node
     */
    @Override
